...
import { BankQuotation, ProductDetail } from '@/types/bank-quotations'

export const generateProfessionalPDF = async (quotation: BankQuotation, productDetails: ProductDetail[]) => {
  ...
  tempDiv.innerHTML = generateDetailedBankQuotationHTML(quotation, productDetails)
  ...
  try {
    ...
    // Download PDF
    pdf.save(`BankQuotation-${quotation.quotation_number}.pdf`)
  } finally {
    // Clean up
...
export const generateDetailedBankQuotationHTML = (quotation: BankQuotation, productDetails: ProductDetail[]): string => {
  ...
  // Apply margin to component prices only (since costs are already adjusted in DB)
  const marginPercentage = quotation.margin_percentage || 0
  const marginMultiplier = 1 + (marginPercentage / 100)
  
  // For summary costs, use database values directly (already adjusted)
  const adjustedRawMaterialCost = quotation.raw_material_cost || 0
  const adjustedInstallationCost = quotation.installation_cost || 0
  const adjustedAdditionalCosts = Math.round((quotation.solar_systems?.additional_costs || 0) * marginMultiplier)
  const adjustedNetAmount = quotation.net_amount || 0
  ...
  // Generate product breakdown table with enhanced details and margin-adjusted pricing
  const productBreakdown = components.map((comp: any) => {
    ...
    // Apply margin to component prices to hide markup
    const adjustedUnitPrice = Math.round((comp.unit_price || 0) * marginMultiplier)
    const adjustedTotalPrice = Math.round((comp.total_price || 0) * marginMultiplier)
    ...
    return `
      <tr>
        ...
        <td style="padding: 15px; border-bottom: 1px solid #ddd; text-align: right; font-size: 15px; font-weight: 500;">${hasValue(comp.unit_price) ? `₹${adjustedUnitPrice?.toLocaleString()}` : 'N/A'}</td>
        <td style="padding: 15px; border-bottom: 1px solid #ddd; text-align: right; font-size: 16px; font-weight: bold; color: #0ea5e9;">${hasValue(comp.total_price) ? `₹${adjustedTotalPrice?.toLocaleString()}` : 'N/A'}</td>
      </tr>
    `
  }).join('')
    ...
    <div style="max-width: 800px; margin: 0 auto; font-family: Arial, sans-serif; color: #333;">
      ...
      <div style="background: #f8f9fa; padding: 30px; border-radius: 8px; border-left: 4px solid #0ea5e9;">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
          <div>
            <div style="margin-bottom: 15px; padding: 15px; background: white; border-radius: 6px;">
              ...
              <div style="font-size: 18px; font-weight: bold; color: #0ea5e9;">₹${adjustedRawMaterialCost?.toLocaleString()}</div>
            </div>
          </div>
          <div>
            <div style="margin-bottom: 15px; padding: 15px; background: white; border-radius: 6px;">
              <strong style="color: #666;">Installation Cost:</strong>
              <div style="font-size: 18px; font-weight: bold; color: #0ea5e9;">₹${adjustedInstallationCost?.toLocaleString()}</div>
            </div>
          </div>
        </div>
        ...
        <div style="margin-bottom: 20px; padding: 15px; background: white; border-radius: 6px;">
          ...
          <div style="font-size: 18px; font-weight: bold; color: #0ea5e9;">₹${adjustedAdditionalCosts.toLocaleString()}</div>
        </div>
        ` : ''}
        ...
        <div style="border-top: 3px solid #0ea5e9; padding-top: 20px; text-align: center;">
          ...
          <div style="font-size: 32px; font-weight: bold; color: #0ea5e9; margin-top: 8px;">
            ₹${adjustedNetAmount?.toLocaleString()}
          </div>
        </div>
      </div>
...
export const printQuotation = async (quotation: BankQuotation, productDetails: ProductDetail[]) => {
  const printContent = generateDetailedBankQuotationHTML(quotation, productDetails)
...
export const generateBankQuotationPDF = generateProfessionalPDF