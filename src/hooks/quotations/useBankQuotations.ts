...
import { BankQuotation, Lead, Vendor, Brand, SolarSystem, ProductDetail, BankQuotationFormData } from '@/types/bank-quotations'

export const useBankQuotations = () => {
  const { message } = App.useApp()
  const [quotations, setQuotations] = useState<BankQuotation[]>([])
  ...
  const createQuotation = async (formData: BankQuotationFormData) => {
    try {
      ...
      // Calculate original and final amounts
      const originalTotal = formData.selectedSolarSystem.total_price || 0
      const marginPercentage = formData.marginPercentage || 0
      const marginAmount = (originalTotal * marginPercentage) / 100
      const finalTotal = originalTotal + marginAmount

      // Distribute margin proportionally across costs
      const equipmentCost = formData.selectedSolarSystem.subtotal_price || 0
      const installationCost = formData.selectedSolarSystem.installation_cost || 0
      const totalBase = equipmentCost + installationCost
      
      const adjustedEquipmentCost = equipmentCost + (marginAmount * (equipmentCost / totalBase))
      const adjustedInstallationCost = installationCost + (marginAmount * (installationCost / totalBase))
      ...
      const quotationData = {
        ...
        // Original amounts (stored for reference)
        total_amount: originalTotal,
        
        // Final amounts with margin applied (these will be displayed in PDF)
        raw_material_cost: Math.round(adjustedEquipmentCost),
        installation_cost: Math.round(adjustedInstallationCost),
        net_amount: finalTotal,
        
        // Bank quotation specific fields
        is_bank_quotation: true,
        margin_percentage: marginPercentage,
        
        valid_until: validUntil,
        status: 'draft' as const,
        notes: marginPercentage > 0 ? `Bank quotation with ${marginPercentage}% margin applied` : '',
  ...
  const updateQuotation = async (id: string, formData: BankQuotationFormData) => {
    try {
      if (!formData.selectedLead || !formData.selectedSolarSystem) {
        throw new Error('Missing required data')
      }
